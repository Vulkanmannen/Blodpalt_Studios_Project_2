...
void InGameMenu::draw(Super &r_super) 
{
	...
	r_super.getWindow().draw(mResumeSprite);
...
void InGameMenu::init()
{
	...
	mResumeTexture.loadFromImage		  ( *ResourceHandler::getInstance()->loadImage("MenuImages/BUTTONS_0000_resume.png"));
	mReturnToMainMenuTexture.loadFromImage( *ResourceHandler::getInstance()->loadImage("MenuImages/BUTTONS_0002_main-menu.png"));
																						
	mResumeHover.loadFromImage			  ( *ResourceHandler::getInstance()->loadImage("MenuImages/BUTTONS_0000_resume_glow.png"));
	...
	mResumeSprite.setPosition				(mMenuSprite.getPosition() + sf::Vector2f(760, 330));
	mResumeTexRect.setTexture				(mResumeTexture);
	mResumeTexRect.setTextureRect			(sf::IntRect(760, 330, 400, 120));
