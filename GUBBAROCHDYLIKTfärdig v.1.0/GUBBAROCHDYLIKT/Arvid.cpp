...
static sf::SoundBuffer WATER_BUFFER;
static sf::SoundBuffer JUMP_BUFFER;
static bool LOADED; 
...
{ 
	if( !LOADED){
		WATER_BUFFER.loadFromFile("Music/1.0GardenGnome-LoveSprinkle2.ogg");
		JUMP_BUFFER.loadFromFile("Music/Alex hopp1.ogg");
		LOADED = true;
	}
	...
	mWaterSound.setBuffer(WATER_BUFFER);
	mJumpSound.setBuffer(JUMP_BUFFER);
	mWaterSound.setVolume(200);
...
void Arvid::onCollision(Entity *e, sf::FloatRect &result)
{
	...
	if(kind == NORMALBLOCK || kind == PUSHABLEBLOCK)
		{
	...
	else if(kind == FLOWER)
	{
		...
		Super::getInstance()->getStateManager().pushState( new LevelClear( EntityManager::getInstance()->getCurrentLevel() ) );

		EntityManager::getInstance()->levelClear( EntityManager::getInstance()->getCurrentLevel() );
	}
	else if(kind == ENEMY){
		Super::getInstance()->getStateManager().pushState( new LevelFail( EntityManager::getInstance()->getCurrentLevel() ) );
	}
}
...
void Arvid::plantFlower(Flower::GrowthDir g)
{
	...
	{
		if(mWaterSound.getStatus() == false)
		{
			mWaterSound.play();
		}
