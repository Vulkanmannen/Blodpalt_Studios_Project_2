...
Flower::Flower(sf::Vector2f position, bool growLeft, GrowthDir dir) :
	...
	mDyingAnimation("dying_flower.png", 100, 11),
	mGrowLeft(growLeft)
{	
	mLifeTime = 300;
	mIsGrowing = true;
	...
	if( mGrowthDir == HORIZONTAL)
	{
		if(mGrowLeft)
		{
			...
			mDyingAnimation.getSprite().rotate( 270 );
		...
		else
		{
			...
			mDyingAnimation.getSprite().rotate( 90 );
		}
	}
}
...
{	
	...
	if( mLifeTime <= 0 )
		mCurrentAnimation = &mDyingAnimation;

	if(mCurrentAnimation == &mDyingAnimation && mCurrentAnimation->endOfAnimation())
		mIsActive = !mIsActive;
}
...
void Flower::grow()
{
	...
	if( mIsGrowing && mGrowthDir == VERTICAL )
	{
		...
		mCurrentAnimation->setPosition( sf::Vector2f(mCurrentAnimation->getSprite().getPosition().x, mCurrentAnimation->getSprite().getPosition().y - 3));
		mIdleAnimation.setPosition( sf::Vector2f(mCurrentAnimation->getSprite().getPosition().x, mCurrentAnimation->getSprite().getPosition().y - 3));
		mDyingAnimation.setPosition( sf::Vector2f(mCurrentAnimation->getSprite().getPosition().x, mCurrentAnimation->getSprite().getPosition().y - 3));
		mDyingAnimation.getSprite().setTextureRect( tempRect );
	...
	else if(mIsGrowing && mGrowthDir == HORIZONTAL)
	{
		...
		if(mGrowLeft)
		{
			...
			mDyingAnimation.setPosition(sf::Vector2f(mCurrentAnimation->getSprite().getPosition().x - 3, mCurrentAnimation->getSprite().getPosition().y));
			mDyingAnimation.getSprite().setTextureRect( tempRect );
